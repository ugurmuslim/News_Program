    ...
    <!-- Notifications Dropdown Menu -->
        <li class="nav-item dropdown">
            <a class="nav-link" href="{{route('article.index',['status' => "ASSIGNED","database" => "maria"])}}">
                ...
                @php
                    $assignedCount = \App\Models\Article::where('editor_id', \Illuminate\Support\Facades\Auth::user()->id)->where('status', \App\Parafesor\Constants\ArticleStatus::ASSIGNED)->count()
                @endphp
                <span
...
<aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="{{route('home.indextest')}}" target="_blank" class="brand-link">
        <img src="{{asset('assets/home/sample/img/logo-icon.svg')}}" alt="Parafesor Logo"
